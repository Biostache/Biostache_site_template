...
export async function generateMetadata({
  ...
  // find the matching static author
  const authorData = authors.find((a) => a.slug === decodedAuthor);
  ...
  return {
    title: `${authorData.name} | Biostache`,
  };
}
...
export default async function AuthorDetails({
  ...
  try {
    // 1) Decode the slug to find the static author record
    const decodedAuthor = decodeURIComponent(params.author);
    const authorData = authors.find((a) => a.slug === decodedAuthor);
    ...
    // 2) Fetch all WP articles and flatten them
    const allArticles = await getArticles();
    const flatArticles = allArticles.flatMap((group) => group.articles);

    // 3) Filter articles that match this author's name
    //    (Adjust the "article.author" comparison if your WP data doesn't match authorData.name exactly)
    const authoredArticles = flatArticles.filter(
      (article) => article.author === authorData.name
    );
    ...
    return (
      <main className="max-w-[95rem] w-full mx-auto px-4 sm:pt-4 xs:pt-2 lg:pb-4 md:pb-4 sm:pb-2 xs:pb-2">
        ...
        <div className="pb-12 md:pb-48">
          ...
          <AuthorArticles articles={authoredArticles} />
        </div>
      </main>
    );
  } catch (error) {
...
function AuthorArticles({ articles }: { articles: SimpleArticleData[] }) {
  return (
